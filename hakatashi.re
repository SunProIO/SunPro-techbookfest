...
ルートサーバーは.jpサブドメイン@<fn>{subdomain}のネームサーバー@<fn>{jpnameservers}を返してきました。
...
#@# サーバーちゃんの住所がわかったクライアントちゃんは、本題に入る前にまず挨拶をする。
...
#@# 自分たちがハガキで文通している事に気づいたクライアントちゃんは、郵便局員に盗み読まれないように、2人だけの秘密の暗号でやり取りしようと提案する。

TLSはTCPのようなコネクションの上で暗号技術を使って通信の機密性や完全性を確保するための仕組みです。
公開鍵暗号を用いて安全に交換した鍵を使って共通鍵暗号を行い、
暗号化されたコネクションをアプリケーションに提供します。

この記事は暗号理論の説明はいたしません。ちょこちょこ出てくる用語の説明はほとんどしていないです。
...
指定されたCipherSuiteにしたがって公開鍵を生成し、次のメッセージに載せます。
そして、自信はその自分の秘密鍵と受け取った公開鍵からpremaster secretを求めます。(Diffie-Hello等)
これによりmaster secretをクライアント側では用意できるようになります。
master secretを用意したらpremaster secretはすぐに抹消します。
もしこの2人のお便りを盗み見してる人に、premaster secretが漏れてしまえば、
そこからmaster secretが計算できるので、あっという間に秘密が破られてしまいます。

=== 4月1日 午前9時39分 ClientKeyExchange, ChangeCipherSpec, Finished 送信

翌朝、クライアントちゃんはサーバーちゃんとの秘密のやり取りに始めてとりかかります。
...
NewSessionTicketでは、セッション再開に必要なサーバーちゃんが持っているべき情報を、
クライアントちゃんに暗号化した状態で持ってもらうための情報です。
実はハンドシェイクの冒頭のHelloで、
お互いにTLS Session Resumption withou Server-Side Stateを利用するかどうかの情報が交換できます。
もしお互いにこれの利用ができるのであれば、セッション再開情報をサーバーちゃんが持たずに、
このチケットという形でクライアントちゃんに持ってもらい、
再開時にサーバーちゃんがそれを受け取り復号化して、再開できます。
が、今回は1回しか通信を行わないので、セッション再開の話はなしにしましょう。

ChangeCipherSpecとFinishedはクライアントちゃんと同じ様な流れです。ちょっと擬似乱数関数に入れる情報が違うだけです。
