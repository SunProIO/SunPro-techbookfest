...
通信内容は、100KBのバイナリファイルをGETリクエストで取得するもので、
...
#@# window size の話
#@# HTTP送信完了時点で受信したACKパケットについて言及

== TLS終了アラートとTCPコネクション切断

#@# 話を終えた二人は、暗号通信の最後のステップを行い、一時の別れを告げる。(手紙で)

=== 9月13日 午前2時51分 HTTPレスポンスの最後のパケット 送信

お別れが近づいてきました。

サーバーちゃんはデータを送る最後のパケットのTCPヘッダにFINフラグを立てて、
クライアントちゃんにデータの終わりを告げます。

=== 10月30日 午前2時29分 HTTPレスポンスの最後のパケット 受信

クライアントちゃんがサーバーちゃんからのお便りを開封すると、
FINフラグが立っていました。
クライアントちゃんはサーバーちゃんにお別れを告げなければなりません。
